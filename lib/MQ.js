...
// Variable that keeps track of the default OAE task exchange
...
// When the connection drops away, try reconnecting every second
connection.setImplOptions({'reconnect': true, 'reconnectBackoffTime': 1000});

// Keep track of whether or not the connection errored just before it closed. If so,
...
connection.on('close', function() {
    if (connectionError) {
        ...
        // set the error flag back to false so we can catch this again when we reconnect
